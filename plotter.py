...
#                print("IS SPIN POLARIZED? ",self._bs.is_spin_polarized)
                proj_br.append(
                    {str(Spin.up): [[] for l in range(self._nb_bands)]})
...
#        print('dictpa_d: %s' % str(dictpa_d))
    ...
    # Change: Added 2 new arguments spin_upcol and spin_upcol
    def get_projected_plots_dots_patom_pmorb(self, dictio, dictpa, spin_upcol='g', spin_downcol='c',
        ...
        print("DICTIO: ", dictio)
        print("DICTPA: ", dictpa)
        print("SUM_ATOMS: ",sum_atoms)
        print("SUM_MORBS: ", sum_morbs)
        ...
        dictpa, sum_atoms, number_figs = self._number_of_subfigures(dictio,
                                                                    ...
                                                                    sum_atoms,
                                                                    sum_morbs)
#        print(number_figs, "of type: ", type(number_figs))
            ...
            for numa in dictpa_d[elt]:
                for o in dictio_d[elt]:
                    ...
                    if num_column is None:
                        if number_figs == 1:
                            pass #plt.subplot(1, 1, 1)    # Change: Replaced with pass
                        else:
                            row = number_figs / 2
                            if number_figs % 2 == 0:
                    ...
                    plt, shift, xmin, xmax = self._maketicks_selected(plt, branches)  #Change: Added 2 new return variables xmin, xmax for x-axis limits
#                    print("SHIFT= ", shift)
                    br = -1
#                    print("BRANCHES= ", branches, "of type ", type(branches))
                    for b in branches:
                        br += 1
                        for i in range(self._nb_bands):
#                            print("B= ", b, "of type ", type(b))
#                            print("BR= ", br,"of type ", type(br))
                            plt.plot(list(map(lambda x: x - shift[br], data['distances'][b])),
                                     [data['energy'][b][str(Spin.up)][i][j]
                                      for j in
                                      range(len(data['distances'][b]))],
                                     'k-', linewidth=band_linewidth)
#                            plt.figure()
#                            print("in here 1")
#                            if i == 3:
#                            return plt
                            ...
                            if self._bs.is_spin_polarized:
                                ...
                                for j in range(len(
                                    ...
                                    plt.plot(
                                        ...
                                        data['energy'][b][str(Spin.down)][i][j],
                                        color=spin_upcol, marker='o', markersize= \
                                            proj_br_d[br][str(Spin.down)][i][j][
                                                elt + numa][o] * 7.0, alpha=0.6)
                            else:
                                spin_downcol=spin_upcol

                            for j in range(
                                    len(data['energy'][b][str(Spin.up)][i])):
                                plt.plot(data['distances'][b][j] - shift[br],
                                         data['energy'][b][str(Spin.up)][i][j],
                                         color=spin_downcol, marker='o', markersize= \
...
# Change: Added the following to specify x-axis limits
            ax=plt.gca()
#            print("AX= ",ax)
    ...
    def _Orbitals_SumOrbitals(self, dictio, sum_morbs):
        ...
        for elt in dictio:
#            print(type(dictio), "has keys: ", dictio.keys())
#            print("ELT= ",elt, "of type: ", type(elt))
            if len(dictio[elt]) == 1:
                if len(dictio[elt][0]) > 1:
                    if sum_morbs is None:
                        pass
                    elif elt in sum_morbs.keys():
                        raise ValueError(
...
#                        print("INDICES:", indices)
        ...
        if (sum_atoms is None) and (sum_morbs is None):
            ...
            for elt in dictpa:
                ...
                if elt in sum_atoms:
                    ...
                    indices = []
                    for i in range(0, len(_sites)):
                        if _sites[i]._species.keys().__eq__(Element(elt)):
                            indices.append(i + 1)
    ...
    def _maketicks_selected(self, plt, branches):
        ...
        """
        ticks = self.get_ticks()
#        print("TICKS= ",ticks, "of type: ", type(ticks))
        xmin=ticks['distance'][0]
        xmax=ticks['distance'][len(ticks['distance'])-1]
        distance = []
        label = []
        rm_elems = []
